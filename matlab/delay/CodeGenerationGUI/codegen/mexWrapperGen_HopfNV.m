...
fprintf(fileID,' if(nrhs!=13) {\n');
fprintf(fileID,'     mexErrMsgIdAndTxt("MyToolbox:GenEigManiPop:nrhs","13 inputs required (some of them are vectors).");\n');
fprintf(fileID,' }\n');
...
fprintf(fileID,' double k = mxGetScalar(prhs[10]);\n');
fprintf(fileID,' double *uPointer = mxGetPr(prhs[11]);\n');
fprintf(fileID,' double *rPointer = mxGetPr(prhs[12]);\n\n\n');
...
fprintf(fileID,' ManifoldEquation(xPointer,alphaPointer,pPointer,omega,w1Pointer,w2Pointer,v1Pointer,\n');
