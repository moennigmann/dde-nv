...
fprintf(fileID,' if(nrhs!=9) {  \n');
fprintf(fileID,'     mexErrMsgIdAndTxt("MyToolbox:DDENLPmodfoldNV:nrhs","9 inputs required (some of them are vectors).");\n');
fprintf(fileID,' }\n');
...
fprintf(fileID,' double k = mxGetScalar(prhs[6]);\n');
fprintf(fileID,' double *uPointer = mxGetPr(prhs[7]);\n');
fprintf(fileID,' double *rPointer = mxGetPr(prhs[8]);\n\n\n');
