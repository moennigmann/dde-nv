...
J=@(x)-6.8e1*abs((x(1)+1i*x(2))+(x(4)+1i*x(5))+(x(7)+1i*x(8)))^2; %Maximale Intensität (mit p1=p2=0.1)
...
%% manually put the fourth constraint to the right mode

 point=aDDENLP.vars.critical(4); 
...
for ii = 1:3
 ...
 aDDENLP.vars.critical(4).x.values=sol.y(:,end)
