...
p=VariableVector(sqrt(2),4,{'distance'});
...
aDDENLP=DDENLP(J,popDDE,xNom,[0;0],[Inf;Inf],[0;0],[10;4],zeros(1,1),30*ones(1,1));
clear popDDE
clear J
clear xNom
...
Aineq = zeros(length(aDDENLP.vars.critical),aDDENLP.occupiedVars);
Aineq(:,aDDENLP.vars.nominal.p.index)=ones(length(aDDENLP.vars.critical),1);


for ii=1:length(aDDENLP.vars.critical)
    Aineq(ii,aDDENLP.vars.critical(ii).l.index)=-1;
end

bineq = zeros(length(aDDENLP.vars.critical),1);

aDDENLP.runOptim(aDDENLP.optionsMainOptim,Aineq,bineq);
