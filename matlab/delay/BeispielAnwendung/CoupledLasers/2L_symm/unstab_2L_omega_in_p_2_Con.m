...
aDDENLP.optionsInitEqCons=optimoptions('fsolve','Algorithm','levenberg-marquardt','MaxIter',10000,'MaxFunEvals',200000,'display','iter','TolFun',1e-12,'TolX',1e-12);
...
history=@(t)[...
    real(E1*1*exp(+1i*omegaDist*t));
    imag(E1*1*exp(+1i*omegaDist*t));
    n1+0*t;
    real(E2*1*exp(-1i*omegaDist*t));
    imag(E2*1*exp(-1i*omegaDist*t));
    n2+0*t];

tEnd=100;

options=ddeset('RelTol',1e-6,'AbsTol',1e-9,'MaxStep',0.01);
sol=ddesd(aDDENLP,point,history,[0 tEnd],options);
