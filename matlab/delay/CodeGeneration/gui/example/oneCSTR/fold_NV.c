...
void mexFunction(int nlhs, mxArray *plhs[], int nrhs, const mxArray *prhs[]) 
{
 ...
 if(nrhs!=5) {  
     mexErrMsgIdAndTxt("MyToolbox:DDENLPmodfoldNV:nrhs","5 inputs required (some of them are vectors).");
 }
 if(nlhs==0) { 
 ...
 plhs[0] = mxCreateDoubleMatrix(1,(mwSize)21,mxREAL);

 /* get a pointer to the real data in the output matrix */
 double *residuumPointer = mxGetPr(plhs[0]);

 /* call the computational routine */
 ManifoldEquation(xPointer,alphaPointer, wPointer,rPointer, residuumPointer);
}