...
fprintf(fileID,'Procedure4CodeGen:=proc(x, alpha,p, w1 )\n');
fprintf(fileID,'m;\n');
...
fprintf(fileID,'CodeGeneration:-C(ManifoldEquation,returnvariablename="residuum",defaulttype=numeric,output="FoldManifold.c",deducetypes=false);\n');
fprintf(fileID,'\n');
% NV
fprintf(fileID,'#pick relevant equations of Normal Vextor System\n');
fprintf(fileID,'manifoldEq:=[]:\n');
...
fprintf(fileID,'end do:\n');
fprintf(fileID,'# create frame for code generation\n');
fprintf(fileID,'Procedure4CodeGen:=proc(x, alpha, p,  w1, r)\n');
fprintf(fileID,'m;\n');
fprintf(fileID,'end proc;\n');
fprintf(fileID,'ManifoldEquation:=subs([m=manifoldEq],eval(Procedure4CodeGen));\n');
fprintf(fileID,'# generate C code\n');
