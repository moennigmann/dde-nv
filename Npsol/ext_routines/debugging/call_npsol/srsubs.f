...
*        ---------------------------------------------------------------
         ...
         if (.not. done) then
            ...
            xtry   = alfa
            if (debug) then
               write(iPrint, 1000) g0    , tolabs, alfmax, 
...
*        ---------------------------------------------------------------
         if (debug) write(iPrint, 1100) alfa, ftry, gtry
...
*           alfa by b-a changes f by less than epsaf.
         ...
         if (debug) then
            write(iPrint, 1200) truea    , trueb , b - a , tol   ,
     $                          nsamea   , nsameb, numf  , 
...
*              ---------------------------------------------------------
               s      = gbest
               q      = gbest - gw
               if (debug) write(iPrint, 2200)
            else
*              ---------------------------------------------------------
*              Fit cubic through  fbest  and  fw.
*              ---------------------------------------------------------
               if (debug) write(iPrint, 2100)
               fitok  = .true.
...
*              bisected.
               ...
               if (debug) write(iPrint, 2400) xtry, daux, dtry
...
*              the artificial interval is just (a, b).
               ...
               xtry   = xmidpt
               if (debug) write(iPrint, 2300) xtry
...
*                 Accept the polynomial fit.
                  ...
                  if (debug) write(iPrint, 2500) xtry
               end if
            end if
         end if
      end if
...
*        ---------------------------------------------------------------
         if (debug) write(iPrint, 1100) alfa, ftry
...
*           alfa by b-a changes f by less than epsaf.
     ...
     $                          alfbst+xw, fw
            if (vset) then
               write(iPrint, 1300) alfbst + xv, fv
            end if
         end if
...
*              Compute the default value of  xtry.
               ...
               if (debug) write(iPrint, 2500) xtry, daux, dtry
...
*              polynomial fit.  Set the default xtry to one tenth xw.
               ...
               if (debug) write(iPrint, 2400) xtry
            end if
...
*                 Accept the polynomial fit.
                  ...
                  if (debug) write(iPrint, 2600) xtry
               end if
            end if
         end if
      end if
