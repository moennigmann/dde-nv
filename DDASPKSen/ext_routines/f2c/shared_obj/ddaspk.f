...
C***REVISION DATE  990309   (Added counter LNLIS=IWORK(39).)
...
C***REVISION DATE  990908   (Replace WRITE and PRINT with XERRWD.)
C***REVISION DATE  991105   (Fixed a bug for initialization of Krylov method.)
C***REVISION DATE  991122   (Fixed a bug for the staggered director method.)
C***CATEGORY NO.  I1A2
C***KEYWORDS  DIFFERENTIAL/ALGEBRAIC, BACKWARD DIFFERENTIATION FORMULAS,
C             IMPLICIT DIFFERENTIAL SYSTEMS, KRYLOV ITERATION
C***AUTHORS   Linda R. Petzold, Peter N. Brown, and Alan C. Hindmarsh
C             Addresses:
...
C            also computed. The class of problems that DASPK can solve 
C            includes index-0, index-1 and Hessenberg index-2 DAE system. 
C            This version is in double precision.
C-----------------------------------------------------------------------
C
C***This version has been modified to perform an optional sensitivity
C   analysis of problem parameters, and to accomodate derivative evaluation 
C   via automatic differentiation (ADIFOR), and to allow parallel 
C   computation of the sensitivities via MPI, and to allow solution of 
C   Hessenberg index-2 DAE systems. All modifications were made by:
C
...
C                   you must provide for the sensitivity computation if you 
C                   select the ADIFOR option to evaluate the sensitivity 
C                   equations. 
C                   
C                   
C *Overview
...
C  directly in the evaluation of the DAE. During the sensitivity analysis, 
C  only the second type of parameters need to be stored. The sensitivities 
C  satisfy the equations -
C
C  dG/dy * s_i + dG/dy' * s'_i + dG/dp_i = 0, i = 1.....Np
C
C  or a finite difference approximation to this equation. The sen -
...
C               |  s_Np  |              |   s'_Np |
C
C  DASPK requires the first type of parameters to be stored following the 
...
C  half-bandwidths in IWORK(1) and IWORK(2). For an ADIFOR-generated 
C  Jacobian, INFO(5)=2 or 3, you must supply the dimension of RPAR in 
C  IWORK(38), if RPAR depends on Y or YPRIME in RES; or set 
C  IWORK(38)=0 if there is no relation between RPAR and Y or YPRIME.
C
...
C
C  *NOTE* This package uses a few subroutines or functions from the ADIFOR 
C         automatic differentiation package and from the MPI parallel 
C         package. If you do not have access to the ADIFOR package, you 
C         can still use DASPK by linking
C            "adf_dummy.f" 
C         file when compiling the code. If you do not have access to the 
C         MPI package, you can still use DASPK by linking 
C            "mpi_dummy.f" 
...
C
C         IRES is an integer flag which is always equal to 0, 1 or 2
...
C         by treating it as dummy argument. If the divided-difference or 
C         ADIFOR method is selected to compute sensitivity equations and 
...
C         and if IRES = 0, RES should evaluate the Ny residuals of the
C         original DAE only. If INFO(20) = 2 and IRES = 1, RES should 
C         evaluate the residuals of the original DAE followed by the residuals
C         of the sensitivity equations, storing them in DELTA of length
C         NEQ = Ny(Np + 1). If INFO(20) = 0 or 1 (finite-difference
C         sensitivity equations), RES should always evaluate the residuals
C         of the original DAE only, storing the result in DELTA of
C         length Ny. If the parallel method is selected (INFO(27)>1) for
C         sensitivity computation, RES should evaluate the residuals of the
C         sensitivity equations with respect to parameters starting from
C         INFO(26)+1, and the ones every other INFO(27) as  
...
C         the residuals of the original DAE only but the index-2 constraints
C         are replaced by their derivatives.
C         
...
C         equations and sensitivity equations, if applicable.
C
...
C            dimensioned of length 30, though DDASPK uses only the 
C            first 27 entries.  You must respond to all of the following
...
C                       to Y.
C                     2. set INFO(5) = 2
C                       and provide the ADIFOR-generated routine in JAC.
C                       ADIFOR with the SparsLinC option should be used.
C                     3. set INFO(5) = 3
C                       and provide the ADIFOR-generated routine in JAC.
C                       ADIFOR with the seed matrix option should be used.
C                                                                ****
C               *Note** If you select INFO(5)=2 or 3, the user
C                       must specify the dependence of RPAR(*) on
C                       Y and YPRIME by setting IWORK(38):
C                       If there is no data dependence, set IWORK(38)=0.
C                       If there is a data dependence, set 
C                       IWORK(38)=size of RPAR(*).
C                       For reasons of storage and efficiency, it is best 
C                       to try to write your code so that it avoids such
C                       a dependency.
C
...
C
C        ***Note: The constraint control acts only on the state variables.
C                 It does not include the sensitivity variables.
C
...
C                    or set INFO(11) = 3 to calculate option 3 ****
C
C                 These options are applicable for index-0, index-1 DAEs 
C                 and for mixed Hessenberg index-2 system where it is not
C                 essential to satisfy the derived constraints at the 
C                 initial values.
C
...
C                  if INFO(10) = 1 or 3.
C
C                  If you have specified INFO(11) = 3, then you must
C                  identity which differential variables or their derivatives
C                  should be fixed by setting:
C
...
C                  The fixed variables must be differential variables.
C
C                  For index-2 problems where it is important that not only 
C                  the original constraints but also the derived constraints 
C                  be satisfied at the initial values, DASPK provides two
C                  other options:
C
C                  INFO(11) = 4. Set this option if the problem is linear 
C                                with respect to the algebraic variables.
C                  INFO(11) = 5. Set this option if the problem is nonlinear
C                                with respect to the algebraic variables.
C
C                  If you have specified INFO(11) = 4 or 5, you must identify
C                  the variables in the same format as for INFO(11) = 3. You
C                  must scale the index-2 constraints by CJ (multiply them by 
C                  CJ) in the RES routine. You will also need to specify which
C                  equations represents index-2 constraints by setting:
C
C                  IWORK(LID+NY+I) = 1, if equation I is an index-2 constraint;
...
C                  classified as algebraic. Sensitivity of a fixed variable
...
C               on next call of DASPK. The pointers of IWORK array might
C               be different from those in the initialization. If you want
C               to continue the integration with the same pointers, set 
C               INFO(14) = 0.
C
...
C
C        INFO(18) -- Not used by this code.
C
...
C
C
C        INFO(20) --options for obtaining the sensitivity equations. 
...
C                Yes - In this case the subroutine RES should define the
...
C                      sensitivities or not in RES routine. 
C                        IF (ires. eq. 0) THEN
C                           compute the state variable residuals only
C                        ELSE IF (ires .eq. 1) THEN
C                           compute both state variable residuals and 
C                           sensitivity residuals
C                        ELSE IF (ires .eq. 2) THEN
C                           compute the state variable residuals only 
C                           for initialization of index-2 systems with
C                           the constraints replaced their derivatives
C                        END IF
C                      RES must be defined differently for different 
C                      IRES if the staggered corrector method (when INFO(25) 
C                      is set to 1) is selected.
C 
...
C                      via ADIFOR. The user should set IWORK(38) according to
C
C                      1. If any element of RPAR depends on Y(*), YPRIME(*),
C                         or SENPAR, set 
C                         IWORK(38) = maximum size of RPAR(*) array.
C                      2. Otherwise, set IWORK(38)=0.
C
...
C                      cannot be chosen.
C
C                   3. Set INFO(5) = 5 and provide an ADIFOR-generated routine
C                      in argument G_RES. This option is only for staggered 
C                      corrector method (INFO(25)=1).
C                      The residuals for the sensitivities will be evaluated 
C                      by matrix times vector methods using the compressed
C                      sparse row format. G_RES should be a routine generated 
C                      by ADIFOR with SparsLinC option and Y, YPRIME, SENPAR
C                      as independent variables. The maximal percentage of the
C                      number of nonzero elements in the matrix (dG/dy, dG/dy')
C                      should be set in RWORK(17). IWORK(38) should be set 
C                      in the same way as for INFO(20)=3 or 4.  ****
C
C
...
C              where offset is defined in INFO(22) and
C              vnorm_i is the 2-norm of the vector v_i defined by v_i(j) =
C              WT(j)/WT(i*Ny + j) for j = 1..Ny, WT(*) is a vector
C              of weights determined by RTOL, ATOL and Y, and cnst
C              is the perturbation factor. The default value for cnst
C              is 1.0d-3. This option allows the user to change this
C              value of cnst.
C
...
C                    desired value.   ****
C
C        INFO(22) --Number of parameters that appear in RES. INFO(22)
C              must be equal to or greater than the dimension of SENPAR(*).
C                        
C        INFO(23) --Error control option for sensitivity variables.
C
C        ****  Do you wish to include the sensitivity variables
C              in the error control test?  Note that DASPK  will
...
C             in between calls to DASPK to find the sensitivity of the 
C             derived quantity Q at the outpout points of DASPK.  
...
C                    of LRW).    ****
C
C       INFO(25) -- Staggered method option for sensitivity analysis.
C
C         **** Do you plan to use staggered method to obtain the 
C              sensitivities?
...
C
C              ***Note***For Krylov method, the staggered director and the 
C                        staggered corrector method are the same.
C
C       INFO(26) -- This option is for parallel computing only. 
C               In parallel computing, each processor has an identity to
C               differentiate itself from others. This identity is usually
...
C               For serial computing, set INFO(26) = 0. 
C
C       INFO(27) -- This option is for parallel computing only.
C               It stores the total number of processors that participate
C               in the parallel computing. 
C               INFO(27) = 0 (our routine will change it to 1) or 1 is for
C               serial computation. In parallel computing (INFO(27)>1),
...
C               RWORK(16) = perturbation factor of finite difference.
C               If INFO(20) = 5 is chosen, set
C               RWORK(17) = The maximum percentage of the number of nonzero
C                           elements in matrix (dG/dy, dG'/dy).
C
...
C              between calls of DASPK, while the rest, which we call ITMP,
C              is just temporally used by DASPK and can be reused by other 
C              program between calls of DASPK.
C
...
C                    if INFO(5) = 3, ITMP1 = NY*(3*NY + IWORK(38))
C                 if INFO(6) = 1 (banded matrix), then add (2*ML+MU+1)*NY.
C                    if INFO(5) = 0, ITMP1 = 2*(NY/(ML+MU+1)+1),
C                    if INFO(5) = 3, ITMP1 = (ML+MU+1)*(3*NY + IWORK(38)).
C              add ITMP = max(ITMP1, ITMP2, ITMP3), where ITMP2 and ITMP3 are
C              defined by
C                 If INFO(11) = 5 (index-2 two-step process)
C                    if INFO(20) < 2, ITMP2 = 2*NY.
C                    if INFO(20) = 3, ITMP2 = 3*NY + INFO(22) + IWORK(38).
C                    if INFO(20) = 4, ITMP2 = 3*NY + INFO(22) + IWORK(38).
C                 Otherwise, ITMP2 = 0
C                 If INFO(19) > 0 (sensitivity analysis)
C                    if INFO(20) < 2, ITMP3 = 4*NY + 2*INFO(24).
C                    if INFO(20) = 3, ITMP3 = 
C                       INFO(19)*(2*NY+MAX(NY,INFO(24))+IWORK(38)+INFO(22)).
C                    if INFO(20) = 4, ITMP3 = IWORK(38)+INFO(22).
...
C              of LIW (or greater).
C              If the ADIFOR option is chosen, and RPAR is used to pass
C              the values of Y, YPRIME or SENPAR, set 
C              IWORK(38) = dimension of RPAR(*)
C
...
C           Otherwise if you choose INFO(5) = 1, then you must supply a 
...
C           initialization of index-2 problem. Depending on the values of
...
C                 AD_OVARS = delta
C                 AD_PREFIX = j
C                 AD_PMAX = a number greater than min(NY, MU+ML+1)
C                 AD_FLAVOR = dense
C
...
C           IER = 0 on entry to JAC, so it needs to be reset only on a failure.
...
C
C  G_RES -- This is the name of an ADIFOR-generated routine you must supply 
...
C                 AD_OVARS = delta
C                 AD_PREFIX = g
C                 AD_PMAX = a number greater than INFO(19)
C                 AD_FLAVOR = dense
...
C
C   Y(*) --contains the computed solution (and sensitivity) approximation at T.
C
C   YPRIME(*) -- contains the computed derivative (and sensitivity derivative)
...
C                        so far.  In the case of sensitivity analysis,
C                        this count includes the number of evaluations
C                        of the state variable system only.
C
...
C                        (Krylov) iterations for state variables so far.
C
...
C  7.  W. F. Feehery, J. E. Tolsma and P. I. Barton, Efficient sensitivity
C      analysis of large-scale differential-algebraic systems}, 
C      Applied Numerical Mathematics 25 (1997), pp. 41-54
C
C
C***ROUTINES CALLED
C
...
C   JDADFSP Jacobian evaluation routine by ADIFOR with SparsLinC option.
C   JDADFSM Jacobian evaluation routine by ADIFOR with seed matrix option.
C   RADFSM Residual evaluation routine by ADIFOR with seed matrix option.
...
C
 ...
 10   CONTINUE
      ITEMP = 5
      IF (INFO(5) .LT. 0. AND. INFO(5). GT. 3) GO TO 701
      DO I=6,9
         ITEMP = I
         IF (INFO(I) .NE. 0 .AND. INFO(I) .NE. 1) GO TO 701
      END DO
      ...
      ITEMP = 11
      IF(INFO(11).LT.0 .OR. INFO(11).GT.5) GO TO 701
      DO 15 I=12,17
         ITEMP = I
         IF (INFO(I) .NE. 0 .AND. INFO(I) .NE. 1) GO TO 701
 15      CONTINUE
...
C
      IF (INFO(5) .EQ. 2 .OR. INFO(20) .GE. 3) THEN
         IF (IWORK(LNRPD) .LT. 0) GO TO 729
      END IF
C
...
C
      ...
      LENID = 0
      IF (INFO(11) .EQ. 1 .OR. INFO(11) .EQ. 3 
     *     .OR. INFO(16) .EQ. 1) LENID = NY
...
C
         ...
         ELSE
            ...
            ELSE
               IWORK(LMTYPE)=8
               MBAND = IWORK(LML)+IWORK(LMU)+1
               ITMP1 = MIN(MBAND,NY)*(3*NY + IWORK(LNRPD))
            ENDIF
         ENDIF
...
C
      ENDIF
      LISE = LENIW+1
      IF (INFO(19) .GT. 0 .AND. INFO(20) .EQ. 5) THEN
C      iwork space for the Sparse Compressed Row format 
         LENIW = LISE + NY + 1 + INFO(22) + RWORK(LNZMX)*(2*NY*NY)
      END IF
C       
      IF(INFO(16) .NE. 0) LENRW = LENRW + MYNEQ
      ITMP2 = 0
      IF (INFO(11) .EQ. 5) THEN
C     Work space for the index-2 initialization
         IF (INFO(20) .LT. 0) ITMP2 = 2*NY
         IF (INFO(20).EQ.3. OR. INFO(20).EQ.4) 
     *        ITMP2 = 3*NY+INFO(22)+IWORK(LNRPD)
      END IF
      ITMP3 = 0
      IF(NP.NE.0) THEN
C     Work space for the sensitivity evaluations
         ...
         ELSE IF (INFO(20) .EQ. 3) THEN
            ITMP3 = NP*(2*NY+MAX(INFO(24),NY)+IWORK(LNRPD)+INFO(22))
         ELSE IF(INFO(20) .EQ. 4) THEN
            ITMP3 = IWORK(LNRPD) + NP  
      ...
      LENRW = LENRW + ITMP
C     Integer work space for ADIFOR with sparseLinC options.
C
      IF ((INFO(12).EQ.0.AND.INFO(5).EQ.2) .OR. INFO(20) .EQ. 5) 
     *     LENIW = LENIW + 3*NY + INFO(22) + IWORK(LNRPD)
C
...
C
      IF (LENID .GT. 0) THEN
         DO 50 I = 1,NY
            IDI = IWORK(LID-1+I)
            IF (IDI.LT.-1 .OR. IDI.GT.3 .OR. IDI.EQ.0) GO TO 724
 50      CONTINUE
...
210      CONTINUE
      DO IP =MYID+1, NP, NUMPROCS
         DO I = 1, NY
            II = IP*NY + I
            IF (INFO(2) .EQ. 1) RTOLI = RTOL(II)
            IF (INFO(2) .EQ. 1) ATOLI = ATOL(II)
            IF (RTOLI .GT. 0.0D0 .OR. ATOLI .GT. 0.0D0) NZFLG = 1
            IF (RTOLI .LT. 0.0D0) GO TO 706
            IF (ATOLI .LT. 0.0D0) GO TO 707
         END DO
...
C
C     Set pointer for IWORK segments
C
      LIADF = LIW - 3*NY - INFO(22) - IWORK(LNRPD)
C
...
C
      ...
      IF (INFO(16) .NE. 0) THEN
        DO I = 1, NY
           IF (IWORK(LID+I-1) .GT. 0) THEN
              RWORK(LVT+I-1) = RWORK(LWT+I-1) 
           ELSE
              RWORK(LVT+I-1) = 0.0D0
           END IF
        END DO
        DO J = 1, NP
           DO I = 1, NY
              IF (IWORK(LID+I-1) .GT. 0) THEN
                 RWORK(J*NY+LVT+I-1) = RWORK(J*NY+LWT+I-1)
              ELSE
...
C-----------------------------------------------------------------------
Cmmo
C      if (idid .eq. -12) print*, "calling error handler after return fro
C     *m DDASIC"
C-----------------------------------------------------------------------
...
410   CONTINUE
      ...
      CALL DDATRP(TN,TOUT,Y,YPRIME,MYNEQ,IWORK(LKOLD),
...
420   IF((TN-T)*H .LE. 0.0D0) GO TO 490
      ...
      CALL DDATRP(TN,TN,Y,YPRIME,MYNEQ,IWORK(LKOLD),
     *  RWORK(LPHI),RWORK(LPSI))
      T = TN
      IDID = 1
      DONE = .TRUE.
...
C
     ...
     *   (ABS(TN)+ABS(H)))GO TO 460
      CALL DDATRP(TN,TSTOP,Y,YPRIME,MYNEQ,IWORK(LKOLD),
     *  RWORK(LPHI),RWORK(LPSI))
      IDID=2
      T=TSTOP
      DONE = .TRUE.
...
C-----------------------------------------------------------------------
Cmmo
C      if (idid .lt. 0) print*, "calling error handler after return from 
C     *DDSTP"
C-----------------------------------------------------------------------
...
C
      ...
      CALL DDATRP(TN,TOUT,Y,YPRIME,MYNEQ,
 ...
 530  IF((TN-TOUT)*H.GE.0.0D0)GO TO 535
      ...
      IDID=1
      GO TO 580
 535  CALL DDATRP(TN,TOUT,Y,YPRIME,MYNEQ,
     *     IWORK(LKOLD),RWORK(LPHI),RWORK(LPSI))
      IDID=3
      T=TOUT
      GO TO 580
...
542   IF(ABS(TN-TSTOP).LE.100.0D0*UROUND*
     *   (ABS(TN)+ABS(H)))GO TO 545
      ...
      H=TSTOP-TN
      GO TO 500
545   CALL DDATRP(TN,TSTOP,Y,YPRIME,MYNEQ,
     *  IWORK(LKOLD),RWORK(LPHI),RWORK(LPSI))
      IDID=2
      T=TSTOP
      GO TO 580
550   IF((TN-TOUT)*H.GE.0.0D0)GO TO 555
      ...
      IDID=1
      GO TO 580
552   CALL DDATRP(TN,TSTOP,Y,YPRIME,MYNEQ,
     *  IWORK(LKOLD),RWORK(LPHI),RWORK(LPSI))
      IDID=2
      T=TSTOP
      GO TO 580
555   CALL DDATRP(TN,TOUT,Y,YPRIME,MYNEQ,
     *   IWORK(LKOLD),RWORK(LPHI),RWORK(LPSI))
      T=TOUT
      IDID=3
580   CONTINUE
...
728   MSG = 'DASPK-- NEQ MUST EQUAL #OF STATE VAR.*(#OF SENS.VAR.+1)'
      ...
      GO TO 750
 729  MSG = 'DASPK-- IWORK(38) MUST BE SET AS SIZE OF RPAR(*)'
      CALL XERRWD(MSG,48,29,0,0,0,0,1,0.D0,0.0D0)
      GO TO 750
...
C
     ...
     *   ISENFO, SENWRK, ISENWK, CNST, SENPAR, G_RES, LIADF)
C
...
C
      ...
      EXTERNAL RES, JAC, PSOL, NLSIC, G_RES
C
      PARAMETER (LCFN=15, LNY=37)
      PARAMETER (LMXNH=34)
C
...
C-----------------------------------------------------------------------
C
C****special treatment for ICOPT = 3, 4, 5
      IF (ICOPT.GT.2 .AND. ICOPT.LT.6) RHCUT = 2.0D0
      MXNH = IWM(LMXNH)
      IDID = 1
      NH = 1
      JSKIP = 0
...
c....for index-2 problems...........................
         IF (ICOPT .EQ. 4 .OR. ICOPT .EQ. 5) THEN
            NY = IWM(LNY)
            IF (ICOPT .EQ. 4) THEN
C....Save the values of YRIME for later use
C
               CALL DCOPY (NEQ, YPRIME, 1, PHI(1,2), 1)
               ICOPT = 7
C
C     Assign YPRIME to zeros
C
               DO I = 1, NEQ
                  YPRIME(I) = 0.0D0
               END DO
...
C
            ...
            END DO
C
C     restore the values for index-2 variables
C
C            DO I = 1, ISENFO(1)+1
C               II = (I-1)*NY
C               DO J = 1, NY
...
C
      IF (ICOPT .EQ. 1 .OR. ICOPT .GT. 5) THEN
C .... ICOPT = 1,6,7
         DO 10 I=1,NEQ
            IF(ID(I) .LT. 0) THEN
               YNEW(I) = Y(I) - RL*P(I)
...
C
      ...
      DIMENSION RPAR(*),IPAR(*)
      DIMENSION ISENFO(*),ISENWK(*), SENWRK(*), SENPAR(*)
      EXTERNAL  RES, JAC, PSOL, NLS, G_RES
C
      PARAMETER (LMXORD=3)
...
C-----------------------------------------------------------------------
Cmmo
C      if (IERNLS .NE. 0 .AND. IERNLS .NE. -2) print*, "calling error han
C     *dler after return from NLS"
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
Cmmo
C      if (ERR .GT. 1.0D0) print*, "calling error handler after return fr
C     *om NLS"
C-----------------------------------------------------------------------
...
665   IF (NEF .GT. 2) GO TO 670
      ...
      R = 0.25D0
Cmmo------------------------------------------------------------
C      print*, "in ddstp, reducing the stepsize by a factor of",
C     * R
Cmmo------------------------------------------------------------
      H = R*H
      IF (ABS(H) .GE. HMIN) GO TO 690
...
C------END OF FUNCTION DDWNRM-------------------------------------------
      END
      DOUBLE PRECISION FUNCTION DDWNRMI(NEQ,V,RWT,RPAR,IPAR,CJ,ICOPT,ID)
C
C***BEGIN PROLOGUE  DDWNRM
C***ROUTINES CALLED  (NONE)
...
C
      ...
      IF (ICOPT .EQ. 1 .OR. ICOPT .GT. 5) THEN
         DO I = 1, NEQ
            IF (ID(I) .GT. 0) V(I) = CJ*V(I)
         END DO
         DDWNRMI = DDWNRM(NEQ,V,RWT,RPAR,IPAR)
         DO I = 1, NEQ
            IF (ID(I) .GT. 0) V(I) = V(I)/CJ
         END DO      
      ELSE
         DDWNRMI = DDWNRM(NEQ,V,RWT,RPAR,IPAR)
      END IF
...
C
     ...
     *        IWM(LNRPD), SENPAR, G_RES)
      ELSE 
         IWM(LNRE) = IWM(LNRE) + 1
         IRES = 0               ! Evaluate the residual only
         CALL RES(X,Y,YPRIME,CJ,DELTA,IRES,RPAR,IPAR,SENPAR)
      END IF
...
C
      ...
      CHARACTER MSG*80
C
      PARAMETER (LNNI=19, LLSOFF=35, LNY=37, LNRPD=38)
...
C
      ...
      RLX = 0.4D0
      NY = IWM(LNY)
      IF (ISENFO(7) .EQ. 0) THEN ! simultaneous corrector method
C>>>>>>>>>>>>>>>>>>>>>>simultaneous corrector method<<<<<<<<<<<<<<<<<<<<
C
C     Compute a new step vector DELTA by back-substitution.
C
         DO I = 1, ISENFO(1)+1
            II = (I-1)*NY + 1
            CALL DSLVD (NY, DELTA(II), WM, IWM)
         END DO
C
C     Get norm of DELTA.  Return now if norm(DELTA) .le. EPCON.
C
         DELNRM = DDWNRM(NY,DELTA,WT,RPAR,IPAR)
         DO I = 1, ISENFO(1)
            II = I*NY + 1     
            TNORM2 = DDWNRM(NY,DELTA(II),WT(II),RPAR,IPAR)
...
C     Check for error condition from linesearch.
         IF (IRET.NE.0 .AND. M.GT.0) GO TO 490
C
...
C
         IF (ICOPT .EQ. 6) THEN
            CALL RES2ND(X,Y,YPRIME,CJ,DELTA,IRES,RPAR,IPAR,SENPAR,
     *           WT,NY,ID(NY+1),RES,G_RES,IWM(LNRPD),CNST,SENWRK,
     *           IWM(LIADF),ISENFO)
         END IF
         CALL DSLVD (NY, DELTA, WM, IWM)
         DELNRM = DDWNRM(NY,DELTA,WT,RPAR,IPAR)
         FNRM = DELNRM
...
C     another iteration.
C
         CALL DCOPY (NY, R, 1, DELTA, 1)
         DELNRM = FNRM      
         GO TO 400
 450     CONTINUE               ! evaluate the sensitivity next
C>>>>>>>>>>>>>>>>>>>>>>>>>>state variable is done<<<<<<<<<<<<<<<<<<<<<<
C
         IF (ISENFO(1) .EQ. 0) RETURN ! no sensitivity
         IF (IWM(LKPRIN) .GE. 2) THEN
            MSG = '------ IN ROUTINE DNSID-- STATE VARIABLES END'
            CALL XERRWD(MSG, 50, 900, 0, 0, 0, 0, 0, 0.0D0,0.0D0)
         ENDIF
         M = 0
         RATE = 1.0D0
         RLX = 0.4D0
...
C
         ...
         DELNRM = FNRM      
         GO TO 460         
      END IF                    ! End of staggered corrector method 
C
...
C-----------------------------------------------------------------------
 ...
 100  CONTINUE
     ...
     *           P(II),ICOPT,ID,YNEW(II),YPNEW(II))
      ...
      END IF
      CALL DFNRMD (
     *     NY, YNEW, X, YPNEW, R, CJ, WT, RES, IRES,
     *     FNRMP, WM, IWM, RPAR, IPAR,
     *     ISENFO, SENWRK, ISENWK, CNST, SENPAR, G_RES, INDEX,
     *     ICOPT, ID, LIADF)
      IF (IRES .LT. 0) THEN
         IRET = 2
         RETURN
...
C All rights reserved.
C
       SUBROUTINE DFNRMD (NY, Y, X, YPRIME, R, CJ, WT, RES, IRES,
     *                   FNORM, WM, IWM, RPAR, IPAR,
     *                   ISENFO, SENWRK, ISENWK, CNST, 
     *                   SENPAR, G_RES, INDEX, ICOPT, ID, LIADF)
C
...
C
      ...
      EXTERNAL RES, G_RES
      DIMENSION Y(*), YPRIME(*), WT(*), R(*)
      DIMENSION WM(*),IWM(*), RPAR(*),IPAR(*), ID(*)
      DIMENSION ISENFO(*), ISENWK(*), SENWRK(*),SENPAR(*)
      PARAMETER (LNRE=12, LNSE=22, LLCIWP=30, LLNIWP=28, LNRPD=38)
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
     ...
     *        IWM(LNRPD),SENPAR, G_RES)
      ELSE 
         ...
         IRES = 0
         IF (ICOPT .NE. 6) THEN
            CALL RES(X,Y,YPRIME,CJ,R,IRES,RPAR,IPAR,SENPAR)
         ELSE
            CALL RES2ND(X,Y,YPRIME,CJ,R,IRES,RPAR,IPAR,SENPAR,
     *           WT,NY,ID(NY+1),RES,G_RES,IWM(LNRPD),CNST,SENWRK,
     *           IWM(LIADF),ISENFO)
         END IF
      END IF
...
C-----------------------------------------------------------------------
      IF (INDEX .GE. 0) CALL DSLVD (NY, R, WM, IWM)
      IF (INDEX .NE. 0) THEN
         DO I = 1, ISENFO(1)
            II = I*NY + 1
            CALL DSLVD (NY, R(II), WM, IWM)
         END DO
      END IF
C-----------------------------------------------------------------------
...
C-----------------------------------------------------------------------
      FNORM = 0.0D0
      IF (INDEX .GE. 0) THEN
         FNORM = DDWNRM(NY,R,WT,RPAR,IPAR)
      END IF
      IF (INDEX .NE. 0) THEN
         DO I = 1, ISENFO(1)
            II = I*NY + 1
            TNORM2 = DDWNRM(NY,R(II),WT(II),RPAR,IPAR)
...
C
      ...
      EXTERNAL  RES, JACD, G_RES
C
      PARAMETER (LNST=11, LNRE=12, LNJE=13, LNSE=22, LNY=37, LNRPD=38)
C
...
330   CONTINUE
      PNORM = DDWNRM (NY,Y,WT,RPAR,IPAR)
      DO I = 1, ISENFO(1)
         II = I*NY + 1     
         TNORM2 = DDWNRM(NY,Y(II),WT(II),RPAR,IPAR)
...
C
      IF(JCALC .EQ. -1 .OR. ISENFO(7) .EQ. 2) THEN
         ...
         IF (JCALC .EQ. -1) THEN
            S = 100.D0
         ELSE
            IF (MOD(IWM(LNST), 20) .EQ. 0) S = 100.D0
         END IF
         JCALC=0
...
C
      ...
      EXTERNAL  RES,G_RES
C
      PARAMETER (LNRE=12, LNNI=19,LNSE=22, LNY=37, LNRPD=38)
...
C
         ...
         DO I = 1, NP
            II = I*NY + 1
            ENORMTMP = DDWNRM(NY,DELTA(ii),WT(ii),RPAR,IPAR)
...
C
 380     CONTINUE
         ...
         ENDIF
         RETURN
      ELSE                      ! staggered corrector method
C>>>>>>>>>>>>>>>>>>>>>>>>>staggered method<<<<<<<<<<<<<<<<<<<
C
...
C
         DELNRM=DDWNRM(NY,DELTA,WT,RPAR,IPAR)
C?         IF (DELNRM .LE. TOLNEW) GO TO 470
         IF (M .EQ. 0) THEN
            OLDNRM = DELNRM
         ELSE
            RATE = (DELNRM/OLDNRM)**(1.0D0/M)
...
C
         ...
         IF (CK*ENORM .GT. 1.0D0) THEN
            ...
            RETURN
         END IF        
         IF (NY .EQ. NEQ) RETURN ! no sensitivity
...
*?         IF (DELNRM .LE. TOLNEW) GO TO 570
         ...
         ENDIF
         IF (DELNRM .LE. TOLNEW) GO TO 570
         IF (S*DELNRM .LE. EPCON) GO TO 570
C
C     The corrector has not yet converged.
C     Update M and test whether the
C     maximum number of iterations have
...
C       the user-supplied routine JACD if IWM(MTYPE) is 1 or 5, or
C       by numerical difference quotients if IWM(MTYPE) is 2 or 6 or
C       by ADIFOR with SparsLinC if IWM(MTYPE) is 3 or 7, or
C       by ADIFOR with SparsLinC if IWM(MTYPE) is 4 or 8.
C       If ADIFOR option is selected, iwork(38) must be specified. 
C       if RPAR(*) depends on Y and YPRIME, set iwork(38)=size of RPAR(*);
...
C     ID       = Array of dimension NEQ, which indicates which variable is
C                algebraic or fixed during the computation.
...
C-----------------------------------------------------------------------
C***ROUTINES CALLED
C   JACD, RES, JDADFSP, JDADFSM, DGEFA, DGBFA
C
...
C
      ...
      MTYPE=IWM(LMTYPE)
      IJAC = 1
      IF (ICOPT .LT. 3) IJAC = 0
      IF (ICOPT .GT. 5) IJAC = 2
...
C     for ICOPT = 3, 4, 5
         ...
         END IF
         IWM(LNRE)=IWM(LNRE)+1
         CALL RES(X,Y,YPRIME,CJ,E,IRES,RPAR,IPAR,SENPAR)
         IF (IRES .LT. 0) RETURN
         DELINV=1.0D0/DEL
         IF (ICOPT .LT. 6) THEN
            DO L=1,NEQ
...
C
 300  CONTINUE
     ...
     2     IWM(LML),ID,ICOPT,JACD,IRES,IWM(LNRPD),E,IWM(LIPVT+NEQ),
     3     IWM(LIADF))
      GO TO 330
C
C     ADIFOR routine to compute the Jacobian, 
...
C
 310  CONTINUE
      ...
      CALL JIDADFSM(
     1     X, Y, YPRIME,WM,CJ,RPAR,IPAR,SENPAR,NEQ,0, IWM(LMU),
     2     IWM(LML),ID,ICOPT,JACD,IRES,IWM(LNRPD),E, WM(LENPD+1))
C
C
C     Do dense-matrix LU decomposition on J.
C
 330  CALL DGEFA(WM,NEQ,NEQ,IWM(LIPVT),IER)
      RETURN
C
C
...
C
 650  CALL DGBFA (WM,MEBAND,NEQ,IWM(LML),IWM(LMU),IWM(LIPVT),IER)
      RETURN
C
...
C
 300  CONTINUE
      ...
      CALL JDADFSP(
     1     X, Y, YPRIME,WM,CJ,RPAR,IPAR,SENPAR,NEQ,0, IWM(LMU),
     2     IWM(LML), JACD, IRES, IWM(LNRPD),E, IWM(LIPVT+NEQ),
     3     IWM(LIADF))
      GO TO 330
C
C     ADIFOR routine to compute the Jacobian, 
...
C
 310  CONTINUE
      ...
      CALL JDADFSM(
     1     X, Y, YPRIME,WM,CJ,RPAR,IPAR,SENPAR,NEQ,0, IWM(LMU),
     2     IWM(LML), JACD, IRES, IWM(LNRPD),E, WM(LENPD+1))
C
C
C     Do dense-matrix LU decomposition on J.
C
...
C
      LIPVT = IWM(LLCIWP)
      MTYPE=IWM(LMTYPE)
      GO TO(100,100,100,100,400,400,400,400),MTYPE
C
C     Dense matrix.
C
100   CONTINUE
...
C
      ...
      DIMENSION WM(*),IWM(*), RPAR(*),IPAR(*)
      EXTERNAL RES, JACK, PSOL, G_RES
C
      PARAMETER (LNRE=12, LNJE=13, LLOCWP=29, LLCIWP=30, LNSE=22)
      PARAMETER (LMXNIT=32, LMXNJ=33, LNY=37, LNRPD=38)
C
C
C     Perform initializations.
C
...
C
     ...
     *        ISENFO,SENWRK,ISENWK,IWM(LNRE),IWM(LNSE),WT,CNST,
     *        IWM(LNRPD),SENPAR, G_RES)
         IF (IWM(LNRPD).GT.0 .AND. ISENFO(1).GT.0) THEN
            IRES = 0
            IWM(LNRE) = IWM(LNRE) + 1
            CALL RES(X,Y,YPRIME,CJ,DELTA,IRES,RPAR,IPAR,SENPAR)
         END IF          
...
C
      IF (JFLG .EQ. 1 .AND. JSKIP .EQ. 0) THEN
         ...
         IWM(LNJE)=IWM(LNJE)+1
         CALL JACK (
     *        RES, IRES, NY, X, Y, YPRIME, WT, DELTA, R, H, CJ,
     *        WM(LWP), IWM(LIWP), IERPJ, RPAR, IPAR,SENPAR)
...
C
      ...
      DIMENSION ICNSTR(*)
      EXTERNAL RES, PSOL, G_RES
C
      PARAMETER (LNNI=19,LNPS=21,LLOCWP=29,LLCIWP=30,LNRE=12,LNSE=22)
      PARAMETER (LLSOFF=35, LNLI=20, LSTOL=14, LNY=37, LNRPD=38)
      PARAMETER (LLNIWP=28, LNLIS=39)
C
C
C     Initializations.  M is the Newton iteration counter.
C
...
C
         CALL DCOPY (NY, DELTA, 1, SAVR, 1)
C
C     Compute a new step vector DELTA.
C
         CALL DSLVK (
     *        NY, Y, X, YPRIME,SAVR,DELTA,WT,WM,IWM,
     *        RES, IRES, PSOL, IERSL, CJ, EPLIN, SQRTN, RSQRTN, 
     *        RHOK, RPAR, IPAR, SENPAR, ICOPT, ID)
         IF (IRES .NE. 0 .OR. IERSL .NE. 0) GO TO 490
         NLI4STAT = IWM(LNLI)
         DO I = 1, NP
            II = I*NY + 1
            CALL DSLVK (
     *           NY, Y, X, YPRIME,SAVR,DELTA(II),WT(II),WM,IWM,
     *           RES, IRES, PSOL, IERSL, CJ, EPLIN, SQRTN, RSQRTN, 
     *           RHOK, RPAR, IPAR, SENPAR, ICOPT, ID)
...
C
     ...
     *        IPAR, ISENFO, SENWRK, ISENWK,CNST, SENPAR, G_RES, 
     *        INDEX, LIADF)
C
         RATE = FNRM/OLDFNM
C
...
C
         ...
         GO TO 400
C---------------------------------------------------------------------
 450     CONTINUE               ! for sensitivity evaluations
C>>>>>>>>>>>>>>>>>>>>>>>state variables are done<<<<<<<<<<<<<<<<<<<<<<
C
         IF (ISENFO(1) .EQ. 0) RETURN
         M = 0
         RATE = 1.0D0
         RLX = 0.4D0
C
...
C
     ...
     *   ISENFO, SENWRK, ISENWK,CNST,SENPAR,G_RES, INDEX, LIADF)
C
...
C
      ...
      CHARACTER MSG*80
C
      PARAMETER (LNRE=12, LNPS=21, LKPRIN=31,LNSE=22,LNY=37,LNRPD=38)
C
      SAVE ALPHA, ONE, TWO
      DATA ALPHA/1.0D-4/, ONE/1.0D0/, TWO/2.0D0/
C
...
C
      ...
      PARAMETER (LNRE=12, LNLI=20, LNSE=22, LNRPD=38, LNLIS=39)
...
C-----------------------------------------------------------------------
      ...
      IF (INDEX .GE. 0) THEN
         CALL DSLVK (
     *        NY, Y, X, YPRIME,SAVR,R,WT,WM,IWM,
     ...
     *        RHOK, RPAR, IPAR, SENPAR, ICOPT, ID)
      ...
      IF (INDEX .NE. 0) THEN
         ...
         DO I = 1, ISENFO(1)
            II = I*NY + 1
            CALL DSLVK (
     *           NY, Y, X, YPRIME,SAVR,R(II),WT(II),WM,IWM,
     *           RES, IRES, PSOL, IER, CJ, EPLIN, SQRTN, RSQRTN, 
     *           RHOK, RPAR, IPAR, SENPAR, ICOPT, ID)
...
C-----------------------------------------------------------------------
      FNORM = 0.0D0
      IF (INDEX .GE. 0) FNORM = DDWNRM(NY,R,WT,RPAR,IPAR)
      IF (INDEX .NE. 0) THEN
         DO I = 1, ISENFO(1)
            II = I*NY + 1     
            TNORM2 = DDWNRM(NY,R(II),WT(II),RPAR,IPAR)
...
C
     ...
     *        ISENFO,SENWRK,ISENWK,IWM(LNRE),IWM(LNSE),WT,CNST,
     *        IWM(LNRPD), SENPAR,G_RES)
         IF (ISENFO(1).GT.0 .AND. IWM(LNRPD).GT.0) THEN
            IRES = 0
            IWM(LNRE)=IWM(LNRE)+1
            CALL RES(X,Y,YPRIME,CJ,DELTA,IRES,RPAR,IPAR,SENPAR)
         END IF          
...
C
      IF(JCALC .EQ. -1)THEN
         ...
         JCALC=0
         IRES = 0
         CALL JACK (RES, IRES, NY, X, Y, YPRIME, WT, DELTA, E, H, CJ,
     *      WM(LWP), IWM(LIWP), IERPJ, RPAR, IPAR, SENPAR)
         CJOLD=CJ
         S = 100.D0
         IF (IRES .LT. 0)  GO TO 380
...
C
      ...
      EXTERNAL  RES, PSOL, G_RES
C
      PARAMETER (LNRE=12, LNNI=19, LNLI=20,LNSE=22,LNY=37,LNRPD=38)
      PARAMETER (LLCIWP=30, LLNIWP=28, LNLIS=39)
...
C     Corrector loop.
C
      IF (ISENFO(7) .EQ. 0) THEN      ! simultaneou corrector method
C>>>>>>>>>>>>>>>>>>>>simultaneous corrector method<<<<<<<<<<<<<<<<<<<<
 300     CONTINUE
         IWM(LNNI) = IWM(LNNI) + 1
C
...
C
     ...
     *        RES, IRES, PSOL, IERSL, CJ, EPLIN, SQRTN, 
     *        RSQRTN, RHOK, RPAR, IPAR, SENPAR, 0, ID)
         IF (IRES .NE. 0 .OR. IERSL .NE. 0) GO TO 580
C
C     Update Y, E, and YPRIME.
C
         DO I=1,NY
...
C
     ...
     1   SQRTN, RSQRTN, RHOK, RPAR, SENPAR
...
C
      IMPLICIT NONE
      INTEGER NEQ, IRES, IWP, IER, NRE, NPSL, IPAR, ICOPT, ID
...
C======================Matrix vector product option for MV=============
      METHOD_MV = 0
      IF (METHOD_MV .EQ. 1) THEN
C---------------set up pointer for IWP---------------------------------
         N2EQP3 = 2*NEQ + 3
         CALL AMUVSP(NEQ,VTEM,Z,WP,IWP(N2EQP3),IWP)
         GOTO 80
      END IF
...
C-----------------------------------------------------------------------
      IF (ICOPT .EQ. 0) THEN
         DO I = 1,NEQ
            ...
            Z(I) = Y(I) + VTEM(I)
         END DO
      ELSE IF (ICOPT.EQ.1 .OR. ICOPT.GT.5) THEN
         DO I = 1,NEQ
            IF (ID(I) .GT. 0) THEN
               Z(I) = Y(I)
               YPTEM(I) = YPRIME(I) + VTEM(I)*CJ
      ...
      ELSE IF (ICOPT.EQ.2) THEN
         DO I = 1,NEQ
            ...
            YPTEM(I) = YPRIME(I)
         END DO
      ELSE IF (ICOPT.EQ.3 .OR. ICOPT.EQ.4 .OR. ICOPT.EQ.5) THEN
         DO I = 1,NEQ
            IF (ID(I) .EQ. 1) THEN
               Z(I) = Y(I) + VTEM(I) 
               YPTEM(I) = YPRIME(I) + VTEM(I)*CJ
...
C-----------------------------------------------------------------------
      CONTINUE
      CALL RES(TN,Z,YPTEM,CJ,VTEM,IRES,RPAR,IPAR,SENPAR)
      NRE = NRE + 1
      IF (IRES .LT. 0) RETURN
C-----------------------------------------------------------------------
...
C------END OF SUBROUTINE DHELS------------------------------------------
      END
      SUBROUTINE DDSEN(T,Y,YP,CJ,DELTA,IRES,RPAR,IPAR,RES,NY,ISENFO,
     *     SENWRK,ISENWK,NRE,NSE,WT,CNST,NRPD,SENPAR,G_RES)
C
      IMPLICIT DOUBLE PRECISION(A-H,O-Z)
...
C     RES and return
C
        CALL RES (T,Y,YP,CJ,DELTA,IRES,RPAR,IPAR,SENPAR)  ! IRES is input
        NSE = NSE + 1
        NRE = NRE + 1
        RETURN
...
C
      SUBROUTINE RES2ND(
     1     T,Y,YPRIME,CJ,DELTA,IRES,RPAR,IPAR,SENPAR,
     2     WT,NY,IDE,RES,G_RES,NRPD,CNST,RWK,IWK,ISENFO)
C================================================================
C    This routine caculates 
C          g_uu' =0 
C    for Hessenberg index-2 problem
C          u'  = f(u,v,t)
C         g(u) = 0
C================================================================
      IMPLICIT NONE
     ...
     *     SENPAR(*), WT(*), CNST, RWK(*)
      INTEGER IRES, IPAR(*), NY, IDE(*),  NRPD, IWK(*), ISENFO(*)
      EXTERNAL RES, G_RES
C === Local variables
      INTEGER I, NP
      REAL*8  VNORM, DEL, DELINV
      ...
      IF (ISENFO(2) .EQ. 2) THEN
         ...
         RETURN
      ELSE IF (ISENFO(2) .EQ. 1 .OR. ISENFO(2) .EQ. 0) THEN
C
...
C     Call RES with perturbed values.
C
         CALL RES(T,RWK,YPRIME,CJ,RWK(NY+1),IRES,RPAR,IPAR,SENPAR)
         IF(IRES.LT.0)RETURN
C
...
C
      ELSE IF (ISENFO(2) .EQ. 3) THEN
C
C     result of ADIFOR with seed matrix option, require 3*ny+np+nrpd rwork 
C     space
C
         call ri2AdfSM(
     1        t, y, yprime, cj, delta, ires, 
     1        rpar, ipar, senpar, ny, np, 
     1        ide, g_res, nrpd, rwk)
C
      ELSE IF (ISENFO(2) .EQ. 4) THEN
C
C     result of ADIFOR with matrix-vector product option, 
C     require 3*ny+np+nrpd rwork space
C
         call ri2AdfMV(
     1        t, y, yprime, cj, delta, ires, 
     1        rpar, ipar, senpar, ny, np, 
     1        ide, g_res, nrpd, rwk)
      ELSE IF (ISENFO(2) .EQ. 5) THEN
C
C     result of ADIFOR with sparcLinC option, require 3*ny+np+nrpd iwork space
C
         call ri2Adfsp(
     1        t, y, yprime, cj, delta, ires, 
     1        rpar, ipar, senpar, ny, np,
     1        ide, g_res, nrpd, iwk)
      END IF 
C
C     Rescale the residuals to the original form
C
      DO I = 1, NY
         IF (IDE(I) .EQ. 1) DELTA(I) = DELTA(I)/CJ
      END DO
      RETURN
C--------------End of RES2ND----------------------------------------
      END
C
     ...
     2     i_res,ires, nrpd, wrk, indvec, iwk)
c=======================================================================
c
...
C=======================================================================   
      ...
      integer nrpd              ! indicator if the rpar(*) depend on
                                ! y(*) or yp(*):
                                ! nrpd = 0 --- independent of y and yp;
                                ! nrpd = maximum length for rpar(*) if depend
                                !
...
*      save iwk
c === local variable
      integer i, j, ig_y, ig_yp, ig_delta,  ig_rpar, width, 
     *     mband, outlen, info, ii,  itotal
      character msg*80
      logical first
      save first
      data first/.true./
...
c
     ...
     $        iwk(ig_delta+1), ires, rpar, ipar, senpar)
         ...
         do i = 1, nrpd
            call dspsd(iwk(ig_rpar+i), 1, 0.0d0, 1)
         end do
         call i_res(
     $        t, y, iwk(ig_y+1), yp,  iwk(ig_yp+1), cj, wrk, 
     $        iwk(ig_delta+1), ires, rpar,iwk(ig_rpar+1),ipar,
     $        senpar)
      end if
c
c      call ehrpt
c
      if (idense .eq. 0) then   ! dense matrix
         do j = 1, ny
            call dspxsq(indvec,wrk,width,iwk(ig_delta+j),
     $           outlen,info)
            do i = 1, outlen
               PD(j+(indvec(i)-1)*Ny)=wrk(i)
            end do
         end do
...
c
      subroutine jidAdfsp(
     1     T,y, yp, PD,CJ,RPAR,IPAR, SENPAR, NY, idense, MU, ML,
     2     id, icopt, i_res,ires, nrpd, wrk, indvec, iwk)
c=======================================================================
c
...
c
      ...
      if (icopt .gt. 2 .and. icopt .lt. 6) then
c ... icopt = 3,4,5
         do i = 1, ny
...
c
c   second stage for index-2 equations
C
         do i = 1, ny
            if (id(i) .gt. 0) then
...
c
      SUBROUTINE jidAdfSM(
     1     T, y, yp, PD,CJ,RPAR,IPAR, senpar, NY, idense, MU, ML,  
     2     id, icopt, j_res, ires, nrpd, delta, wrk)
c=======================================================================
c
...
C=======================================================================   
      ...
      real*8  wrk(*)            ! work array, size = width*(3*NY+nrpd)
c === local variable
      integer i, j, ig_y, ig_yp, ig_delta, ipos, width, ll,ul, 
     *     length, mebm1, ii,jj, itotal, ig_rpar
c
c === define the width of the seed matrix
c
      if (idense .eq. 1) then
         WIDTH=min(ny,1+ML+MU)
      else 
         width = ny
      end if
c
...
c
      ...
      ig_rpar = ig_delta + length
      itotal = ig_rpar + nrpd*width
c
...
c
      ...
      DO I=1,Ny
         ...
         else if (icopt .gt. 2 .and. icopt .lt. 6) then
            if (id(i) .lt. 2) then
               wrk(ig_y + ipos )=1.0D0
               wrk(ig_yp+ ipos )=cj
            else if (id(i) .eq. 2) then
...
c
      if (nrpd .eq. 0) then
         call j_res(
     *        width, t, y, wrk(ig_y+1), width, yp, wrk(ig_yp+1), 
     *        width, cj, delta,  wrk(ig_delta+1), width,
     *        ires, rpar, ipar, senpar)
      else 
         call j_res(
     *        width, t, y, wrk(ig_y+1), width, yp, wrk(ig_yp+1), 
     *        width, cj, delta,  wrk(ig_delta+1), width,
     *        ires, rpar, wrk(ig_rpar+1), width, ipar, senpar)
      end if        
c
      if (icopt .lt. 6) then
         if (idense .eq. 1) then
            ...
            do i=1, Ny
               ipos = (I-1)*Ny
               do j=1,Ny
                  PD(I + (J-1)*Ny )=wrk(ig_delta+ j + ipos)
               end do
            end do
         end if
         return
...
c === local variable
     ...
     1           cj,delta,delta(1+i*Ny),ires,rpar,ipar,senpar,wrk)
         ...
         do i = 1, isenfo(1)
            ...
            if (myi .le. isenfo(4)) wrk(myi) = 1.0d0
            call g_res(t,y,y(1+i*Ny),yp,yp(1+i*Ny),
     1           cj,delta,delta(1+i*Ny),ires,rpar,wrk(npp1),ipar,
     2           senpar,wrk)
            if (myi .le. isenfo(4)) wrk(myi) = 0.0d0
         end do
      end if
...
C
     ...
     1     rpar, ipar, senpar, ny, np, 
     1     ide, g_res, nrpd, wrk)
C==============================================================
C
C    This routine calculates the residual of 
C         g_u u' = 0
C    for index-2 problem via ADIFOR with matrix-vector product
C    only method
C
C==============================================================
C
      ...
      integer  ires, nrpd       ! other parameters
      real*8   wrk(*)           ! work array at least 3*ny + np + nrpd
c === local variables
      integer  i, j, isenpar,ig_y, ig_yp, ig_delta, ig_rpar, itotal,
     *         ig_senpar
C           
      ig_senpar = 0
      ig_y  = ig_senpar + np
      ...
      if (nrpd .eq. 0) then
         call g_res(
     *        t, y, wrk(ig_y+1), yp, wrk(ig_yp+1), 
     *        cj, delta,  wrk(ig_delta+1),
     *        ires, rpar, ipar, senpar, wrk)
      else 
         call g_res(
     *        t, y, wrk(ig_y+1), yp, wrk(ig_yp+1), 
     *        cj, delta,  wrk(ig_delta+1),
     *        ires, rpar, wrk(ig_rpar+1), ipar, senpar,wrk)
      end if        
c         
      do i = 1, ny
         if (ide(i) .eq. 1) then
            delta(i) = wrk(ig_delta + i)
         end if
      end do
c
      return
...
C===================================================================
      ...
      real*8  wrk(*)            ! work array, 
                                !size = isenfo(1)*(3*ny+nrpd+isenfo(4))
c === local variable
      integer i, ig_rpar, np, ig_y, ig_yp,ig_delta, 
...
c
     ...
     *        np, cj, delta, wrk(ig_delta+1), np, ires,
...
C    This routine calculates the residual of 
C         g_u u' = 0
C    for index-2 problem via ADIFOR with seed matrix method
C
C==============================================================
      implicit none
      real*8   t                ! time
      ...
      real*8   wrk(*)           ! work array at least 3*ny + np + nrpd
c === local variables
      integer  i, ig_senpar, ig_y, ig_yp, ig_delta, ig_rpar, itotal
      ...
      if (nrpd .eq. 0) then
         call g_res(
     *        1, t, y, wrk(ig_y+1), 1, yp, wrk(ig_yp+1), 
     *        1, cj, delta,  wrk(ig_delta+1), 1,
     *        ires, rpar, ipar, senpar, wrk, 1)
      else 
         call g_res(
     *        1, t, y, wrk(ig_y+1), 1, yp, wrk(ig_yp+1), 
     *        1, cj, delta,  wrk(ig_delta+1), 1,
     *        ires, rpar, wrk(ig_rpar+1), 1, ipar, senpar,wrk, 1)
      end if        
c         
      do i = 1, ny
         if (ide(i) .eq. 1) then
...
C
     ...
     1     t, y, yp, cj, delta, ires, 
     1     rpar, ipar, senpar, ny, np,
     1     ide, i_res, nrpd, iwk)
C==============================================================
C
C    This routine calculates the residual of 
C         g_u u' = 0
C    for index-2 problem via ADIFOR with SparsLinC method
C
C==============================================================
      implicit none
      real*8   t                ! time
      ...
      integer  ny               ! number of variables 
      integer  np               ! number of sensitivities
      external i_res            ! Adifor-generated routine
      integer  ires, nrpd       ! other parameters
      integer  iwk(*)           ! integer work space 
c === local variable
      integer i, j, ig_y, ig_yp, ig_delta,  ig_rpar, ig_senpar, 
     *        outlen, info, itotal
      integer indvec(2)
      character msg*80
      logical first
...
c === initialize the sparselink 
      ...
      do i = 1, ny
         call dspsd(iwk(ig_y+i), i, 0.0d0, 1)
         call dspsd(iwk(ig_yp+i), j, yp(i), 1)
      end do
      do i = 1, np
         call dspsd(iwk(ig_senpar+i), i, 0.0d0, 1)
      end do
c
c === call Adifor generated routine
c
      if (nrpd .eq. 0) then
         call i_res(
     $        t, y, iwk(ig_y+1), yp,  iwk(ig_yp+1), cj, delta, 
...
c
      do j = 1, ny
         if (ide(j) .eq. 1) then
            call dspxsq(indvec,delta(j),1,iwk(ig_delta+j),
     $           outlen,info)  
            if (info .lt. 0) then
               msg = 'DASPK-- ERROR IN Ri2ADFSP'
               CALL XERRWD(MSG,25,40,0,0,0,0,1,0.D0,0.0D0)
               ires = info
               RETURN
            end if
...
C      
     ...
     2     IPAR, RPAR, SENPAR, IRES, DFDP, NP, IWK, NRPD)
c==================================================================
...
C ... Input arguments:
      ...
      integer nrpd              ! indicator on whether rpar depends on Y.
...
c ... Workspace for  adifor with sparslink
      integer iwk(*)            ! integer work array,size>=3*ny+np+nrpd
c === local variable
      integer i, j, ig_y, ig_yp, ig_delta, ig_rpar, width, jj, itotal
